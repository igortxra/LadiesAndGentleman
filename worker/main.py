...
async def process_payment(msg_id, payload, client: httpx.AsyncClient, processor_name: str) -> Tuple[bool, float]:
    requested_at = datetime.now(timezone.utc)
    payload['requestedAt'] =  requested_at.strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3] + "Z"
    score = requested_at.timestamp()
    response = await client.post(f"{PROCESSOR_URL[processor_name]}/payments", json=payload, timeout=10)
            ...
            for _, entries in msgs:
                for msg_id, payload in entries:
                    ...
                    while not message_processed:
                        response = await process_payment(msg_id, payload, client, processor_name)
...
async def worker(processor_name: str, consumer_name: str):
    ...
    async with httpx.AsyncClient(limits=limits) as client:
        while True:
            ...
            stream_batch = await REDIS.xreadgroup(groupname='payment-workers', consumername=consumer_name, streams={STREAM_NAME: '>'}, count=batch, block=0)
            
            tasks = []
            for _, entries in stream_batch:
                for msg_id, payload in entries:
                    tasks.append(process_payment(msg_id, payload, client, processor_name))
...
async def main():
    ...
    try:
        await REDIS.xgroup_create(name=STREAM_NAME, groupname="payment-workers", id="0", mkstream=True)
    except Exception:
    ...
    workers = [*default_workers, *fallback_workers, health_check("default", 0), health_check("fallback", 2.5)]
